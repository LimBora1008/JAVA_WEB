...
/*
	회원 가입 처리 서블릿
	- 클라이언트에서 JSON Type 문자열 형태로 값을 받아옴
	- 구글의 Gson 라이브러리를 통해 JSON Type 문자열을 자바 객체로 매핑(바인딩) / 구글 gson 라이브러리 import 해야함
	- Member vo 클래서 필요
 */
...
public class InsertAndReturn extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 3. jsonData 문자열을 Member 자바 객체로 매핑
		Member member2 = gson.fromJson(jsonData.toString(),	Member.class); // fromJson : json문자열로 받은 객체를 Member객체로 자동 매핑해줌(자바객체)
		System.out.println("member : "+member2.toString()); // [디버깅]
