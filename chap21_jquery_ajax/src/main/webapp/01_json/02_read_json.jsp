...
$(document).ready(function() {
	// 아이디가 btn이고 그 자식들 중에서 btnLoad 클래스를 찾아서 
	// click 이벤트 부여
	$('#btn > .btnLoad').click(function(){
		/*
			...
			- 응답으로 오늘 파일이 반드시 json형태일 경우만 사용
			
			[콜백함수] : 서버에 어떤 요청을 보내면 그 요청이
			성공 / 실패 하고 난 후 자동으로 불려지는 함수를
			콜백함수 라고 한다. function(data){...}
		...
		*/
		$.getJSON('${contextPath}/01_json/json_data.js',function(data){
			...
			// html의 body부분에 나타낼 html tag 생성
			// table의 제목 부분
			var table = "<table border='1'><tr><td>인덱스</td><td>번호</td><td>이름</td></tr>";
			/*
				- data : 읽어들인 json_data
				- json data 가 갖고 있는 값을 each문으로 반복(index:순번 , entry:값)
