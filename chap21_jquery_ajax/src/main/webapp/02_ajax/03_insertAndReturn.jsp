...
<script>
	$(document).ready(function() {
		...
		$("#changeBtn").click(function(e) {
			...
			// 3. JSON.stringify(obj) 스트링지 파이 함수
			...
			// ajax 형태로 서블릿 호출(서버에 값 전달)
			 $.ajax({
               ...
               type : 'post',              // 요청방식 (get, post, put 등등)
                ...
                dataType : 'text',          // 전달받을 데이터타입
				success : function(result){ // 결과 성공 콜백함수
				console.log('서버 처리결과 받은 jsonData : '+result);
				
				// 1. 서버에서 받은 json 타입 문자열을 자바스크립트 객체로 변환
				...
				error : function(request,status,error) { // 결과 에러 콜백
				console.log(error)
				}
...
<body>
	<h3>화면 입력값을 JSON 문자열로 변환하여 서버에 전달,<br>서버에서는 자바 객체를 JSON 문자열로 응답</h3>
	<h4>
		1. [전송]버튼을 클릭하면 자바스크립트 부분에서 입력값을<br>
			JSON 타입 문자열로 변환하고<br>
		2. jQuery의 ajax 함수를 통해서 서블릿으로 전송<br> 
		3. 서블릿은 전송된 데이터가 json type의 문자열이므로<br>
			문자열을 자바 객체로 변환하여 처리하는게 편리<br>
		4. 서버에서 웹브라우저로 전송하기 전에 자바 쪽에서 만든 객체를 다시<br>JSON Type 문자열로 변환하여 사용자에게 반환한다<br>
		5. 사용자의 자바스크립트에서는 서버에서 받은 JSON Type문자열을 <br> 자바스크립트 객체로 파싱(변환)하여 사용
	</h4>
	...
	<div>
		...
		반환 비밀번호 : <input type="password" id="returnPwd"><br> 
	</div>
</body>
</html>